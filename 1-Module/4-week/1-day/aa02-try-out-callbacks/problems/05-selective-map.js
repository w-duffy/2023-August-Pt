...
function selectiveMap(array, selector, mapper) {
    ...
    for (let i = 0; i < array.length; i++){
        let el = array[i]
        let resultOfCb = selector(el)

        if(resultOfCb){
            //if firstcallback === true then you call the second callback
            // console.log(el)
            let mappedEl = mapper(el)
            // console.log(mappedEl)
            newArr.push(mappedEl)
        } else {
            newArr.push(el)
        }
    }
